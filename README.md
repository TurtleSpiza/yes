...
- `toolkit/branch/` the branch register toolkit: `pbr_stage.py` (load, inherit, identify from creditor histories, classify, capture), `pbr_build.py` (write, recalc, verify, ship), `pbr_capture.py` (rule 16/17 capture from a gated corpus), `pbr_rules_v1.json` (classification rules as data), `pbr_histories_v4.json` (APLEDGER creditor histories as data: file, code, label, ABN), `parse_mixed1.py`, `parse_mixed_new.py`, `parse_attach1.py` and `parse_attach2.py` (raw-text invoice parsers), `prep_code_corpus.py` (prepares and fidelity-checks a supplied third-party corpus), `pbr_recon_batch.py` (the Document Reconstruction journal route, rule 21), `pbr_unidentified_queue.py` and `pbr_journal_pull_report.py` (the identification queue and journal pull list reports).
...
## State at v10 (14-Sep-2026)

Register total $4,910,566.68 across 6,683 lines, tied to the ledger export and all four SE2 views; 99 of 99 controls TRUE. 349 sighted lines over 342 invoices. Twenty-six APLEDGER creditor histories embedded (72,596 lines on Creditor_Lines). Confirmed $3,507,438.12; Partial $1,064,221.11; Pending evidence $338,907.45. Contractor unidentified $264,171.42 over 164 lines in 75 series; the queue with one invoice to sight per series is `reports/Unidentified_Contractors_v10.md`.
