...
| A17a | v7.2 to v7.7 | **`header_sources` had no defined shape for a DERIVED figure.** P17 exempted a derived figure from the value test and not from the row-exists limb, so an extractor with a derived GST had no defined value to write and whatever it invented could fail. | `prompt_audit.py` Class E, and four hand reviews before it that raised it and never closed it. | Open **four releases**, longer than anything else in the schema, and never named in a release. | v7.8: the shape `{"basis": "derived", "row": null}`, exempt from both limbs, in section 9 and 13.1 and in gate v12. |
| A17b | v7.2 to v7.8 | **v7.2 had no annexe naming it**, and it is the largest amendment in this standard's history: P16, P17, F8 and `gst_basis`. Annexe D covered it and its heading named no version. | `prompt_audit.py` Class A, reverse direction. | No mechanical check could map the release to its annexe. | v7.8: Annexe D retitled. |
| A17c | v7.6 to v7.8 | **4.6's tie assert used float `abs()`** where 6.0 mandates `Decimal` with `ROUND_HALF_UP`. | Audit Class D. | An extractor copying the fast path literally ties in binary floating point, where a tie at exactly 1c is not reliably within tolerance. | v7.8: `Decimal` with `copy_abs()`. |
| A17d | v7.7 to v7.8 | **The description-layer qualifier had no row in 13.2's scoping table** while `archival` had one. | Audit Class L. | The qualifier discipline's whole content is that a qualifier gates nothing, and the one qualifier whose first version DID gate something, at a cost of GREEN on 36 of 37 corpora, was the one that could not be shown not to. | v7.8: the row. |
| A17e | v7.6 to v7.8 | **Nine releases share 18-Sep-2026 and v7 is dated after v7.1**, so the history cannot be ordered by date. | Audit Class C, carried three releases. | A reader ordering by date gets the sequence wrong. | v7.8: an explicit `seq` table, 1 to 9, and the dates now carry nothing. |
...
| B10a | `pswp_money_screen.py` | **It did not read `manifest.archival`**, which the gate had read since v11. | Audit Class L, testing the qualifier discipline across every tool rather than the one it was written in. | A reviewer running the screen straight over `batches/` screened 11 snapshots as though live: the exact defect rule 11.12 exists to stop, one tool later. | v2 reads it and prints `[ARCHIVAL]`. |
| B11 | the Glascott corpus | **`"supplier_abn": ""`**, an empty string where the field is absent. | Counting distinct ABNs for the audit and getting 33 where the verification report had 32. | It produced audit finding #27, which was **wrong in both halves** and published. An empty string is not an absent field and no check currently says so. | Open, carried as audit #34. |
| B12 | `prompt_audit.py` | **Its line count used `str.split("\n")`**, which returns one extra empty element on a file ending in a newline, so it reported 1,200 against `wc -l`'s 1,199. | Writing the corrected count into the assessment and finding the two tools disagreed. | It would have replaced a count wrong by one with a count wrong by one the other way, in the same release that corrected it. | Fixed: the auditor counts as `wc -l` counts. |
...
| C9 | The prompt was "946 lines", then "962", then "1,027", then "1,146". | Each was stated from memory rather than from `wc -l`. The fourth was found by the auditor, against 1,147. | The web session twice, then `prompt_audit.py`. Line counts are now read off the file in the same command that reports them, and the auditor counts the way `wc -l` counts. |
| C10 | "F8 fired on 97 of 100 documents in `Binder1666`" in the assessment, against "68 documents" in the prompt's F8 row. | **Neither.** 96, recomputed against gate v12. | `prompt_audit.py` Class I. Two figures for one quantity, both printed as current, is now a mechanical check. |
| C11 | Annexe D2's addendum: "four of the 7 RED are archival snapshots". | Five. Annexe D5, 13.0 and the assessment all said five, in the same document, written the same day. | `prompt_audit.py` Class F. |
| C12 | Audit finding #27: "the live corpora carry 33 ABNs against 32 verified", naming Woodman Beenleigh Pty Ltd as never checked. | **Wrong in both halves.** `ABN_Verification_v24.xlsx` carries that ABN, and the 33rd value was an empty string (B11). | Myself, building the v25 verification input and finding 32. Withdrawn in the same release that raised it. |
| C13 | The audit report claimed its fixtures run with `--fixture`-equivalent `--root`. | They did not; the mode did not exist. | Running the command in the report. The mode was then written so the claim could be true, and a clean fixture now exits 2 so a fixture that stops testing is visible. |
...
**A fourth pattern, visible only once the log was long enough.** Six of the nine reporting errors are a
**figure stated from memory rather than recomputed**: C6, C7, C8, C9, C10, C11. Every one of them was in a
document that opens by saying every figure in it is reproducible. `prompt_audit.py` now recomputes them, and
the rule that came out of it is in the audit prompt's Class I: **a published figure carries a sweep date, a
denominator and the live/archival split**, or it cannot be compared to its successor and will be superseded
silently.
